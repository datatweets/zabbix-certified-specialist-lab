...
troubleshooting capability. **That is the Certified Specialist skill set.** This module
...
**Natural next step:** book the **ZCS exam** if you haven't (Modules 37–38), then aim
...
The course covered each of these at a *foundational* level (often as a lab shortcut);
here is where they go in the real world:
...
When you tackle a new domain, the *method* you learned transfers directly:
...
cluster, or a fleet of routers.

### A monitoring strategy for real organizations

Tools don't make good monitoring — **strategy** does:
...
- **End on momentum, not a full stop.** The Specialist cert is a milestone; frame the
  advanced areas as the exciting next chapters, each anchored to something they already
  did in this course.
- **Make the roadmap personal.** The worksheet is the deliverable — push everyone to
  fill **§7** with a *dated* first step (book the exam, build one template at work).
  Vague intentions fade; a date sticks.
- **Right-size ambition.** Discourage "monitor everything / go straight to Expert."
  Strategy first (services/SLA), then the next rung (ZCP), then scale.
- **Reinforce the transferable method.** New domain → understand source → pick collector
  → template → discover → alert/report. This is what makes a Specialist effective
  beyond the topics they were taught.
- **The lab is the gift.** Reiterate: keep and extend the Docker lab. Most people lose
  skills because they stop touching the tool; a resettable lab solves that.
- **Lab vs production — final word.** Everything in this course maps up: containers →
  real/cloud/k8s hosts, single server → HA cluster, local SMTP → enterprise alerting,
  manual config → IaC. The *thinking* you practised is the production thinking.
- **Timing (~45 min):** ~10 min what-you-built recap + certification ladder, ~15 min
  advanced areas + transferable method + strategy, ~15 min fill the roadmap worksheet,
  ~5 min share roadmaps + course close.

## Lab-State Delta

Module 39 (what's next — knowledge module, no lab objects created):

- **Committed artifact:** `content/exam/monitoring-roadmap-worksheet.md` — a fillable
  roadmap (current skills, next certification, advanced topics, org needs, architecture,
  automation/IaC, a 30/90/180-day plan, resources).
- Certification roadmap facts grounded in official zabbix.com (ZCP: requires ZCS, 3-day,
  HA/encryption/advanced LLD/event correlation/perf tuning; ZCE: requires ZCP, 5-day,
  DB tuning at scale/upgrades/vaults/LDAP-SAML-MFA/TimescaleDB/SELinux). No live-lab
  changes. Lab at 8 hosts. **Course complete — all 40 modules authored.**
