...
- **Lab vs production — PSK.** PSK is ideal for a handful of hosts or a branch. At
  fleet scale, certificates (PKI) scale better and can be revoked, at the cost of
  expiry management. Zabbix supports both; the menu and host fields are the same.
  Mention that a PSK can also be set at the host-group level via a template.
- **The reconnect backoff catches everyone.** After enabling PSK, the host's
  interface is briefly unavailable, and the server then waits out its
  unreachable-retry delay (~60s) before trying again with the key. Students think
  it is broken; it is just backoff. Show the interface flip back to available.
- **`zabbix_get` is the proof tool.** The before/after `zabbix_get` (refused, then
  `1` with the key) is the single most convincing demonstration that encryption is
  actually enforced, not just configured.
- **Lab vs production — database TLS.** A common misconception is that "MySQL 8
  encrypts by default." The *server* supports TLS, but encryption only happens if
  the *client* asks for it — and the Zabbix server image links MariaDB Connector/C,
  which does not. So in this lab the server↔DB link is plain `TCP/IP` (we verify
  that with `performance_schema.threads`), and we prove the channel is available by
  forcing `--ssl-mode=REQUIRED`. The before/after is a good teaching moment: do not
  assume a link is encrypted — check. To *enforce and verify* it the way the
  reference book (Ch12) does, you would, on a real external database:
  - Build a private CA and issue a server cert with **OpenSSL**, configure MariaDB/
    MySQL with `ssl-ca`, `ssl-cert`, `ssl-key`, and `require_secure_transport=ON`.
  - Configure the Zabbix server with `DBTLSConnect=verify_full`, `DBTLSCAFile`,
    `DBTLSCertFile`, `DBTLSKeyFile`, and require the `zabbix` DB user with
    `ALTER USER 'zabbix'@'%' REQUIRE SSL`.
  - We do **not** force `verify_full` in the lab because the auto-generated
    MySQL CA changes whenever the database volume is reset (`down -v`), which would
    break a committed CA file — a good illustration of why production uses a
    *stable* private CA, not auto-generated certs.
- **Note on `ZBX_DBTLSCONNECT`.** This server image does not substitute the
  `ZBX_DBTLSCONNECT` environment variable into its config (it leaves the
  `${ZBX_DBTLSCONNECT}` template literal), so enforcement is configured directly
  in `zabbix_server.conf` on a real install rather than via the env var here.
- **Timing (~40 min):** ~10 min why-encrypt + PSK vs certs, ~18 min PSK end-to-end
  with the refused/accepted proof, ~8 min database TLS verify + production
  enforcement, ~4 min recap.

## Lab-State Delta

Added in Module 46 (kept):

- **PSK key:** `content/lab/agent-psk/agent.psk` (64-hex, identity `PSK001`),
  mounted into `zabbix-agent-basic` at `/var/lib/zabbix/enc/agent.psk`.
- **Agent change:** `zabbix-agent-basic` now requires PSK (`ZBX_TLSCONNECT`/
  `ZBX_TLSACCEPT` = `psk`, identity `PSK001`).
- **Host change:** `zabbix-agent-basic` (hostid `10780`) — `tls_connect` = `2`
  (PSK), `tls_accept` = `2`, identity `PSK001`, key set. Monitoring verified over
  PSK (unencrypted `zabbix_get` refused; encrypted succeeds; interface available).
- **Database:** verified the server↔`zabbix-db` connection is plain `TCP/IP` by
  default (the Alpine server's MariaDB Connector/C client does not negotiate TLS),
  and proved the encrypted channel is available by forcing `--ssl-mode=REQUIRED`
  (cipher `TLS_AES_128_GCM_SHA256`). Production `verify_full`+CA enforcement
  documented, not forced.
- **No new containers.** Screenshots in `content/extra/assets/module-46/`.
