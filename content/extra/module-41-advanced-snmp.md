...
- **Lab vs production.** `demo-snmp-device` is net-snmp standing in for a
  router/switch; the SNMPv3 mechanics (user, SHA/AES, security level) are exactly
  those of a real Cisco/Juniper device. In production you would link a vendor SNMP
  template (which already uses bulk walks and LLD) rather than hand-build items.
- **Test from the CLI first — always.** Step 2 is the single most valuable habit
  in SNMP troubleshooting. If `snmpget -v3` fails, the problem is the device or
  the credentials, not Zabbix. The most common v3 mistakes are a passphrase under
  8 characters, the wrong auth/priv protocol, or the wrong security level.
- **Secret macros are one-way.** Once a macro is set to *Secret text*, the UI and
  API will not show its value; you can only overwrite it. Mention this so students
  are not confused when the value reads as dots forever.
- **Why bulk matters.** Drive the point home with arithmetic: a 48-port switch
  with 10 metrics per port is 480 single gets per interval the old way, or ~10
  bulk walks the new way. That is the difference between a healthy poller and a
  saturated one (tie back to Module 13's queue).
- **`createUser` lifecycle.** net-snmp consumes `createUser` at startup and moves
  the user into its persistent state. Because this container is rebuilt from the
  image (no persistent `/var/lib/net-snmp` volume), the user is recreated on every
  start, which is what we want for a reproducible lab.
- **Timing (~40 min):** ~12 min v3 security model + secret macros, ~13 min build
  and verify the v3 interface/item, ~12 min bulk walk + dependent, ~3 min recap.

## Lab-State Delta

Added in Module 41 (kept — extends the Module 20 SNMP device):

- **Device change:** `content/lab/demo-snmp-device/snmpd.conf` now defines an
  SNMPv3 user — `createUser snmpv3user SHA "authpass123" AES "privpass123"` +
  `rouser snmpv3user authpriv`. Image rebuilt; v2c (`public`) unchanged.
- **Host `demo-snmp-device` (hostid `10793`):**
  - **Macros added:** `{$SNMPV3_SECNAME}`=`snmpv3user`, `{$SNMPV3_AUTH}` (Secret),
    `{$SNMPV3_PRIV}` (Secret).
  - **Interface added:** SNMPv3 interface (interfaceid `90`) — authPriv, SHA1,
    AES128, security name `{$SNMPV3_SECNAME}`.
  - **Items added:** `snmp.v3.sysname` (`72309`, v3, =`demo-snmp-device`);
    `snmp.walk.ifdescr` (`72310`, SNMP OID `walk[1.3.6.1.2.1.2.2.1.2]`, master);
    `snmp.walk.ifdescr.eth0` (`72311`, dependent, **SNMP walk value** OID
    `1.3.6.1.2.1.2.2.1.2.11`, =`eth0`).
- **No new containers.** Screenshots in `content/extra/assets/module-41/`.
