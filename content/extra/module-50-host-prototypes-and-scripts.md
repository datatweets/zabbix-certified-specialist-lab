...
- **Lab vs production.** We simulate a CMDB/cloud feed with `zabbix_sender`. In
  production the same host-prototype machinery is driven by the built-in *VMware*,
  *AWS by HTTP*, *Azure by HTTP*, and *Kubernetes* templates, which discover real
  instances and create hosts automatically (see Module 13 of the reference book
  for the cloud variants).
- **`{HOST.ID}` is not available here.** A natural first attempt is to pass
  `{HOST.ID}` to the webhook, but that macro is not resolved in manual host-action
  scripts — which is why the Enable/Disable scripts resolve the host by *name*
  (`host.get` on `{HOST.HOST}`) before updating it. Good teachable moment about
  verifying macro availability rather than assuming it.
- **Least-privilege tokens.** The toggle scripts hold an API token. Stress that it
  should belong to a **scoped** user (Module 25), not Super Admin, and that
  storing it as a script parameter keeps it out of the script body. In production,
  rotate it like any credential.
- **Trapper LLD timing.** A trapper-based discovery rule processes when data
  arrives, so the hosts appear within moments of the `zabbix_sender` call — no
  polling interval to wait on. Lost hosts age out per the rule's lifetime.
- **`{MANUALINPUT}` validation is enforced server-side.** Show the `9` rejection
  so students see the validator is real protection, not just a UI hint — it stops
  a fat-fingered value before the script runs.
- **Timing (~40 min):** ~15 min host prototypes (rule, prototype, feed, verify),
  ~10 min `{MANUALINPUT}` script + validation, ~12 min enable/disable scripts and
  running them from a map, ~3 min recap.

## Lab-State Delta

Added in Module 50 (kept):

- **Host group:** `Discovered Shop VMs` (groupid `29`).
- **LLD rule:** `Shop VM discovery` (`lld.vms`, Zabbix trapper, ruleid `72314`) on
  `zabbix-agent-basic` (10780), with a **host prototype** `{#VMNAME}` (custom Agent
  interface IP `{#VMIP}`).
- **Discovered hosts:** `shop-vm-01` (`10.0.0.31`), `shop-vm-02` (`10.0.0.32`,
  hostid `10801`) — created by LLD from the `zabbix_sender` feed.
- **API token:** `frontend-scripts-token` (Admin user) used by the toggle scripts.
- **Frontend scripts (Alerts → Scripts):** `Online Shop/Ping host (manual count)`
  (scriptid `4`, Webhook, `{MANUALINPUT}` prompt + `^[1-5]$` validator);
  `Online Shop/Host Disable monitoring` (`5`); `Online Shop/Host Enable monitoring`
  (`6`) — both Webhook, scope Manual host action, toggle `status` via `host.update`.
- **No new containers.** Screenshots in `content/extra/assets/module-50/`.
