...
- **Lab vs production.** The agent 2 host here runs *inside* Docker with the
  socket mounted read-only. On a real Docker host you would install agent 2 on the
  host itself and add the `zabbix` user to the `docker` group
  (`gpasswd -a zabbix docker`); the template and keys are identical. Stress the
  socket as a sensitive, host-controlling resource — read-only, least privilege.
- **Why "Get info" looks empty.** The most common student question. The master
  `docker.info` item has *Do not keep history* set; its value is consumed by
  dependent items via preprocessing. Show a dependent (Containers running) to
  prove collection is healthy. This mirrors the master/dependent lesson in
  Module 9.
- **Config-cache reload.** Linking a template does not start collection
  instantly; the server polls new items only after a config cache reload (≤60s by
  default). Use `-R config_cache_reload` to avoid "why is there no data yet?"
  confusion.
- **Discovery interval.** The Containers/Images discovery rules run on their own
  interval (not every minute); per-container items may take a little longer to
  appear than the engine metrics. In this lab they appeared promptly because the
  engine is small.
- **Tie-in.** This is the payoff of a Docker-based course: the platform monitors
  *itself*. Good moment to connect back to Module 6 (agent 2 plugins) and
  Module 23 (LLD).
- **Timing (~35 min):** ~10 min concept (engine vs workload, agent 2 plugin,
  master/dependent), ~15 min link + verify engine and container metrics, ~10 min
  discussion of socket security and production deployment.

## Lab-State Delta

Added in Module 43 (kept — deepens the existing Docker monitoring):

- **Host change:** `zabbix-agent2-docker` (hostid `10781`) — linked template
  **`Docker by Zabbix agent 2`** (templateid `10318`) in addition to its existing
  *Linux by Zabbix agent* template.
- **Items added:** ~40 template items including master `docker.info` (history
  disabled) and dependents `docker.containers.total` (=19), `docker.containers.running`
  (=15), `docker.images.total` (=33), `docker.ping` (=1).
- **Discovery rules added:** **Containers discovery** (`docker.containers.discovery`,
  `{#ID}`/`{#NAME}`), **Images discovery** (`docker.images.discovery`), plus the
  template's net.if/vfs discovery rules. Container LLD created per-container item
  sets for every running container (incl. all `demo-*` Online-Shop systems).
- **No new containers.** Uses the existing `zabbix-agent2-docker` socket mount.
  Screenshots in `content/extra/assets/module-43/`.
